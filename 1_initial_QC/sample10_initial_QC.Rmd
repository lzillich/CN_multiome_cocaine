...
counts <- Read10X("/path/to/cellranger_out/Sample10/outs/filtered_feature_bc_matrix/")
...
sample <- CreateSeuratObject(counts = counts$`Gene Expression`,
                               assay = "RNA",project = "sample10")
sample[['ATAC']] <- CreateChromatinAssay(counts = counts$`Peaks`,
                                           annotation = annotations,
                                           fragments = paste0("/path/to/cellranger_out/Sample10/outs/atac_fragments.tsv.gz"),sep = c(":", "-"),genome = 'hg38')
...
sample <- doubletFinder_v3(sample, PCs = 1:10, pN = 0.25, pK = 0.13, nExp = nExp_poi.adj, reuse.pANN = FALSE, sct = FALSE)

DefaultAssay(sample)<-"RNA"

DF.name1 = paste0("DF.classifications_", 0.25, "_", 0.13, "_", nExp_poi.adj)
...
saveRDS(sample,"/path/to/data/preprocessing/sample10.rds")
