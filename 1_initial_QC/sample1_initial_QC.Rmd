...
counts <- Read10X("path/to/cellranger_out/Sample1/outs/filtered_feature_bc_matrix/")
...
sample <- CreateSeuratObject(counts = counts$`Gene Expression`,
                               assay = "RNA",project = "sample1")
sample[['ATAC']] <- CreateChromatinAssay(counts = counts$`Peaks`,
                                           annotation = annotations,
                                           fragments = paste0("path/to/cellranger_out/Sample1/outs/atac_fragments.tsv.gz"),sep = c(":", "-"),genome = 'hg38')
...
sweep.res.list <- paramSweep_v3(sample, PCs = 1:10)
...
sample <- doubletFinder_v3(sample, PCs = 1:10, pN = 0.25, pK = 0.01, nExp = nExp_poi.adj, reuse.pANN = FALSE, sct = FALSE)

DefaultAssay(sample)<-"RNA"

# Überprüfung der Ergebnisse
p1 <- DimPlot(sample, reduction = "umap", group.by = paste0("DF.classifications_", 0.25, "_", 0.01, "_", nExp_poi.adj))

p2 <- FeaturePlot(sample, features = "nFeature_RNA" )

p1 + p2

DF.name1 = paste0("DF.classifications_", 0.25, "_", 0.01, "_", nExp_poi.adj)
...
saveRDS(sample,"/path/to/data/preprocessing/sample1.rds")
