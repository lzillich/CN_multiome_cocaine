...
seurat <- readRDS(file = "/path/to/data/5_MOFA.rds")
```
...
p4 <- plot_factor(mofa, factors=6, group_by = "celltypes",color_by="CUD",add_dots = F,add_violin  = T,dodge = T,violin_alpha  = 1,add_boxplot = T) +theme(strip.text.x = element_blank())+
  theme(
    axis.text.x = element_text(color="black", angle=40, vjust=1, hjust=1)
  )

ggsave("/path/to/figures/FigureS4b_Vlnplot_F6.pdf",p4,width=6,height=6)
...
# F6
f6_GSEA <- data.frame(TERM=rep("",times=20),ES=rep(0,times=20), p_adj=rep(0,times=20),dir=c(rep("up",times=10),rep("down",times=10)))

f6_GSEA$p_adj[1:10] <- pos_padj$Factor6[order(pos_padj$Factor6,decreasing = F)]
f6_GSEA$TERM[1:10] <- rownames(pos_padj[order(pos_padj$Factor6,decreasing = F),])[1:10]
f6_GSEA$p_adj[11:20] <- neg_padj$Factor6[order(neg_padj$Factor6,decreasing = F)]
f6_GSEA$TERM[11:20] <- rownames(neg_padj[order(neg_padj$Factor6,decreasing = F),])[1:10]

pos_terms <- pos_ES[rownames(pos_ES) %in% rownames(pos_padj[order(pos_padj$Factor6,decreasing = F),])[1:10],]
pos_terms <- pos_terms[match(rownames(pos_padj[order(pos_padj$Factor6,decreasing = F),])[1:10],row.names(pos_terms)),]
neg_terms <- neg_ES[rownames(neg_ES) %in% rownames(neg_padj[order(neg_padj$Factor6,decreasing = F),])[1:10],]
neg_terms <- neg_terms[match(rownames(neg_padj[order(neg_padj$Factor6,decreasing = F),])[1:10],row.names(neg_terms)),]

f6_GSEA$ES[1:10] <- pos_terms$Factor6
f6_GSEA$ES[11:20] <- -(neg_terms$Factor6)

# rename pathways and round pvals for plotting
f6_GSEA$TERM <-  gsub("GOBP_","",f6_GSEA$TERM)
f6_GSEA$TERM <-  gsub("_"," ",f6_GSEA$TERM)
f6_GSEA$TERM <-  tolower(f6_GSEA$TERM)
f6_GSEA$p_adj <- signif(f6_GSEA$p_adj, digits=3)


f6_gsea_plot <- ggbarplot(f6_GSEA, x = "TERM", y = "ES",
          fill = "dir",               # change fill color by cyl
          color = "black",            # Set bar border colors to white
          sort.val = "desc",          # Sort the value in dscending order
          sort.by.groups = FALSE,     # Don't sort inside each group
          #x.text.angle = 90           # Rotate vertically x axis texts
          )+ylim(-10,10)+NoLegend()+xlab(NULL)+ylab("enrichment score")+geom_text(aes(y=2*abs(ES)/ES,label = p_adj), vjust = 0.5,hjust=0.5,size=2.5)+  theme(axis.text.x = element_text(size = 14))+coord_flip()

ggsave("/path/to/figures/FigureS4d_GSEA_factor6.pdf",f6_gsea_plot,width=8,height=8)
...
f7_gsea_plot <- ggbarplot(f7_GSEA, x = "TERM", y = "ES",
          ...
          sort.by.groups = FALSE,     # Don't sort inside each group
          #x.text.angle = 90           # Rotate vertically x axis texts
          )+ylim(-12,12)+NoLegend()+xlab(NULL)+ylab("enrichment score")+geom_text(aes(y=2*abs(ES)/ES,label = p_adj), vjust = 0.5,hjust=0.5,size=2.5)+  theme(axis.text.x = element_text(size = 14))+coord_flip()
...
f8_gsea_plot <- ggbarplot(f8_GSEA, x = "TERM", y = "ES",
          ...
          sort.by.groups = FALSE,     # Don't sort inside each group
          #x.text.angle = 90           # Rotate vertically x axis texts
          )+ylim(-10,50)+NoLegend()+xlab(NULL)+ylab("enrichment score")+geom_text(aes(y=5*abs(ES)/ES,label = p_adj), vjust = 0.5,hjust=0.5,size=2.5)+  theme(axis.text.x = element_text(size = 14))+coord_flip()
...
```{r motif6,warning=FALSE, message=FALSE, results='hide'}
...
sig.motifs.positive_distal <- motif.enrichment.positive_distal$pval.adj[,"Factor6"] %>%
  sort %>% head(n=4) %>% names
sig.motifs.positive_promoter <- motif.enrichment.positive_promoter$pval.adj[,"Factor6"] %>%
  sort %>% head(n=4) %>% names
sig.motifs.negative_distal <- motif.enrichment.negative_distal$pval.adj[,"Factor6"] %>%
  sort %>% head(n=4) %>% names
sig.motifs.negative_promoter <- motif.enrichment.negative_promoter$pval.adj[,"Factor6"] %>%
  sort %>% head(n=4) %>% names
...
ggsave("/path/to/figures/Figure3/3e_Motif_plots_neg_distal_8.pdf",p15c,width=5,height=2.5)
p15d <- MotifPlot(seurat, motifs = sig.motifs.negative_promoter,assay="ATAC_promoter")
ggsave("/path/to/figures/Figure3/3e_Motif_plots_neg_promoter_8.pdf",p15d,width=5,height=2.5)
```
