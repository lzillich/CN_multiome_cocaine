...
#seurat <- readRDS("/path/to/data/4_cell_types_harmony_24.rds")
...
#saveRDS(seurat,"/path/to/data/6_LinkedPeaks.rds")
...
pdf("/path/to/figures/Figure2b_Upset_DEG.pdf",width=6,height=4)
...
p3 <- emapplot(both_dir_GO,legend_n=3,cex_line=0.1,cex_label_category=0.85,layout="nicely",cex_category=0.8,showCategory=16,pie="count")+scale_fill_manual(values=c("#F7AB64","#74BA59","#70305A", "#E8326D", "#3A9BCC","#85CEE4","#003E65","#BFE1D7","#D1BCDC"))
ggsave("/path/to/figures/Figure2c_GO_both_dir_new.pdf",p3,height=8,width=8)
ggsave("/path/to/figures/Figure2c_GO_both_dir_new_legend.pdf",p3,height=18,width=18)
...
p4 <- emapplot(KEGG,legend_n=3,cex_line=0.1,cex_label_category=0.85,layout="nicely",cex_category=0.8,showCategory=16,pie="count")+scale_fill_manual(values=c("#F7AB64","#74BA59","#70305A", "#E8326D", "#3A9BCC","#85CEE4","#003E65","#BFE1D7","#D1BCDC"))
  ggsave("/path/to/figures/Figure2d_KEGG_multiome.pdf",p4,height=8,width=8)
...
pdf("/path/to/figures/Figure2eDAc_hm.pdf",width=3,height=8)
...
for (i in anno$Feature){
  df <- data.frame(i=c(anno_merged$sum[anno_merged$region == "peaks" & anno_merged$Feature == i],anno_merged$sum[anno_merged$region == "DA" & anno_merged$Feature == i]),other=c(204152-anno_merged$sum[anno_merged$region == "peaks" & anno_merged$Feature == i],13482-anno_merged$sum[anno_merged$region == "DA" & anno_merged$Feature == i]))
...
anno_merged$region[12:22] <- rep("DA peaks - CUD vs. Ctrl",times=11)
anno_merged$region <- factor(anno_merged$region , levels = c("background","DA peaks - CUD vs. Ctrl"))
...
names(genes_prom) = c("OPC","Oligodendrocyte","Astrocyte","Microglia","GABAergic_D1MSN","GABAergic_D2MSN","GABAergic_CALB2","GABAergic_D1MSN_ADARB2")       
GO_prom <- compareCluster(geneCluster   = genes_prom,
                         fun           = "enrichGO",
                         pvalueCutoff  = 0.05,
                         OrgDb=org.Hs.eg.db)
GO_prom <-pairwise_termsim(GO_prom)
p3 <- emapplot(GO_prom,legend_n=3,cex_line=0.1,cex_label_category=0.75,layout="nicely",cex_category=0.8,showCategory=12,pie="count")+scale_fill_manual(values=c("#70305A","#F7AB64", "#85CEE4","#BFE1D7"))
...
p3_1 <- emapplot(GO_dist,legend_n=3,cex_line=0.1,cex_label_category=0.75,layout="nicely",cex_category=0.8,showCategory=12,pie="count")+scale_fill_manual(values=c("#E8326D","#F7AB64", "#74BA59","#3A9BCC","#D1BCDC","#003E65"))
...
saveRDS(seurat, "/path/to/data/5_post_DE.rds")
```
