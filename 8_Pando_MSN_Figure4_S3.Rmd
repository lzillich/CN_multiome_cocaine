...
can_genes <- fread("/path/to/results/DE/FindMarkers/clusterGABAergic - D1 MSN.csv") #use gene list of choosing

#read in transcription factors
tfs <- read.table("/path/to/Multiome_Tutorial/Homo_sapiens_TF.txt", sep="\t", header=T)


df_pfm <- data.frame(t(sapply(pfm, function(x)
  c(id=x@ID, name=x@name, symbol=ifelse(!is.null(x@tags$symbol),x@tags$symbol,x@name))))) # unclear in vignette
...
plot_gof(MSN, point_size=3)

plot_module_metrics(MSN)
...
ggsave("/path/to/results/Pando/MSN.pdf",p1,width=6,height=6)
...
quantile(unname(sort(table(grn$target),decreasing = T)),probs=c(0.05,0.25,0.5,0.75,0.95))
# 5% 25% 50% 75% 95% 
# 1   2   5   9  17 
...
for(i in colnames(regulons)[c(4:length(colnames(regulons)))]){
  ...
  reg_CUD <- regulons[,i][regulons$CUD=="CUD"]
...
reg <- ggplot(res_df, aes(regulon, score,label=regulon)) +
  geom_point(size=0.5)+geom_text_repel(hjust=0, vjust=0,max.overlaps=3, nudge_y = 0.1, size = 3.5,data = subset(res_df,lab=="yes"))+ylab("regulon size * foldChange in CUD")+ theme(axis.title.x=element_blank(),
...
p1 <- lapply(p1,function(p){
  p+xlim(c(-12.5,-5))+ylim(c(-8,8))+NoAxes()
})
library(patchwork)
...
ob <- readRDS(file = "/path/to/data/6_LinkedPeaks.rds")
...
ggsave("/path/to/results/Pando/MSN_CACNA1C.pdf",p_final,width=5,height=5)
...
library(patchwork)
p_final <- p_cov / p_an / p_pe / p_mot + plot_layout(heights=c(5,2,2,1.5)) & theme(text = element_text(size=10))
ggsave("/path/to/results/Pando/MSN_PDE10A.pdf",p_final,width=5,height=5)
...
DGI <- read.delim("/path/to/results/Pando/DGIdb/gene_interaction_results-4_4_2024.tsv")
